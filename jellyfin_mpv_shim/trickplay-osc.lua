...
---@type boolean
local thumb_is_shown
mp.register_script_message("osc-thumb-is-shown", function(val)
    thumb_is_shown = val == "true"
end)

---@type number
local thumb_overlay_id = 46
mp.register_script_message("osc-thumb-overlay-id", function(val)
    thumb_overlay_id = tonumber(val) or 46
end)
        ...
        if element.type == "slider" then
            ...
            if pos then
                ...
                if slider_lo.stype ~= "bar" then
                    ...
                    ass_draw_rr_h_cw(elem_ass, xp - r, foH - r, xp + r, foH + r, r, slider_lo.stype == "diamond")
                else
                    local h = 0
...
function bar_layout(direction)
    ...
    geo = { x = osc_geo.x + padX + padwc_l, y = line2, an = 4, w = buttonW, h = 36 - padY * 2 }
    lo = add_layout("playpause")
    lo.geometry = geo
    ...
    -- Fullscreen button
    geo = { x = osc_geo.x + osc_geo.w - buttonW - padX - padwc_r, y = geo.y, an = 4, w = buttonW, h = geo.h }
    lo = add_layout("tog_fs")
    lo.geometry = geo
    lo.style = osc_styles.smallButtonsBar
...
mp.add_key_binding("DEL", "visibility", function()
