...
local img_count = 0
local img_multiplier = 0
local img_width = 0
local img_height = 0
local img_file = ""
local img_last_frame = -1
local img_is_shown = false
local img_enabled = false
local img_is_bif = false
local img_chapters = {}
local img_overlay_id = 46

mp.commandv("script-message", "osc-thumb-overlay-id", tostring(img_overlay_id))

local function show_img(val)
    img_is_shown = val
    mp.commandv("script-message", "osc-thumb-is-shown", tostring(val))
end

local function send_thumbfast_message()
...
local function client_message_handler(event)
    ...
    elseif event_name == "shim-trickplay-bif" then
        ...
        img_is_bif = true
        send_thumbfast_message()
    elseif event_name == "shim-trickplay-chapters" then
